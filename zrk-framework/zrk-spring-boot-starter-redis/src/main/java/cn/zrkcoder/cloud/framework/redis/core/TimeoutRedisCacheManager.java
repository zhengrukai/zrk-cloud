package cn.zrkcoder.cloud.framework.redis.core;
...
public class TimeoutRedisCacheManager extends RedisCacheManager {
    ...
    /**
     * 创建一个 RedisCache 实例，支持通过名称中的分隔符指定自定义过期时间。
     *
     * <p>该方法重写了父类的 createRedisCache 方法，增加了对缓存名称中自定义过期时间的支持。
     * 如果缓存名称中包含特定分隔符（如 #），则解析其中的时间部分并设置为缓存的过期时间；
     * 否则使用默认配置创建缓存。</p>
     *
     * @param name 缓存名称，可能包含自定义过期时间的标识（例如 "cacheName#10s"）
     * @param cacheConfig Redis 缓存的基础配置对象，可为 null
     * @return 返回创建的 RedisCache 实例
     */
    @Override
