package cn.zrkcoder.cloud.framework.jackson.config;

import cn.zrkcoder.cloud.framework.common.util.json.JsonUtils;
import cn.zrkcoder.cloud.framework.common.util.json.databind.NumberSerializer;
import cn.zrkcoder.cloud.framework.common.util.json.databind.TimestampLocalDateTimeDeserializer;
import cn.zrkcoder.cloud.framework.common.util.json.databind.TimestampLocalDateTimeSerializer;
...
@Slf4j
public class ZrkJacksonAutoConfiguration {
    ...
    public Jackson2ObjectMapperBuilderCustomizer ldtEpochMillisCustomizer() {
        return builder -> builder
                // Long -> Number 使用 NumberSerializer.INSTANCE 序列化，避免前端 JavaScript 精度丢失。
                .serializerByType(Long.class, NumberSerializer.INSTANCE)
                .serializerByType(Long.TYPE, NumberSerializer.INSTANCE)
                // LocalDate / LocalTime 使用 JSR-310 提供的标准序列化器和反序列化器
                ...
                // LocalDateTime < - > EpochMillis 实现时间戳（毫秒）与 LocalDateTime 的双向转换。
