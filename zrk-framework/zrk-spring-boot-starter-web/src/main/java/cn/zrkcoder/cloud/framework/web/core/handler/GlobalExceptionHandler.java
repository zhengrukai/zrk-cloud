package cn.zrkcoder.cloud.framework.web.core.handler;
...
import cn.zrkcoder.cloud.framework.common.exception.ServiceException;
import cn.zrkcoder.cloud.framework.common.exception.util.ServiceExceptionUtil;
import cn.zrkcoder.cloud.framework.common.pojo.CommonResult;
import cn.zrkcoder.cloud.framework.common.util.collection.SetUtils;
...
import static cn.zrkcoder.cloud.framework.common.exception.enums.GlobalErrorCodeConstants.*;
...
public class GlobalExceptionHandler {
    ...
    private CommonResult<?> handleTableNotExists(Throwable ex) {
        ...
        if (message.contains("report_")) {
            log.error("[报表模块 zrk-module-report - 表结构未导入][参考 https://cloud.zrkcoder.cn/report/ 开启]");
            return CommonResult.error(NOT_IMPLEMENTED.getCode(),
                    "[报表模块 zrk-module-report - 表结构未导入][参考 https://cloud.zrkcoder.cn/report/ 开启]");
        }
        // 2. 工作流
        if (message.contains("bpm_")) {
            log.error("[工作流模块 zrk-module-bpm - 表结构未导入][参考 https://cloud.zrkcoder.cn/bpm/ 开启]");
            return CommonResult.error(NOT_IMPLEMENTED.getCode(),
                    "[工作流模块 zrk-module-bpm - 表结构未导入][参考 https://cloud.zrkcoder.cn/bpm/ 开启]");
        }
        // 3. 微信公众号
        if (message.contains("mp_")) {
            log.error("[微信公众号 zrk-module-mp - 表结构未导入][参考 https://cloud.zrkcoder.cn/mp/build/ 开启]");
            return CommonResult.error(NOT_IMPLEMENTED.getCode(),
                    "[微信公众号 zrk-module-mp - 表结构未导入][参考 https://cloud.zrkcoder.cn/mp/build/ 开启]");
        }
        // 4. 商城系统
        if (StrUtil.containsAny(message, "product_", "promotion_", "trade_")) {
            log.error("[商城系统 zrk-module-mall - 已禁用][参考 https://cloud.zrkcoder.cn/mall/build/ 开启]");
            return CommonResult.error(NOT_IMPLEMENTED.getCode(),
                    "[商城系统 zrk-module-mall - 已禁用][参考 https://cloud.zrkcoder.cn/mall/build/ 开启]");
        }
        // 5. ERP 系统
        if (message.contains("erp_")) {
            log.error("[ERP 系统 zrk-module-erp - 表结构未导入][参考 https://cloud.zrkcoder.cn/erp/build/ 开启]");
            return CommonResult.error(NOT_IMPLEMENTED.getCode(),
                    "[ERP 系统 zrk-module-erp - 表结构未导入][参考 https://cloud.zrkcoder.cn/erp/build/ 开启]");
        }
        // 6. CRM 系统
        if (message.contains("crm_")) {
            log.error("[CRM 系统 zrk-module-crm - 表结构未导入][参考 https://cloud.zrkcoder.cn/crm/build/ 开启]");
            return CommonResult.error(NOT_IMPLEMENTED.getCode(),
                    "[CRM 系统 zrk-module-crm - 表结构未导入][参考 https://cloud.zrkcoder.cn/crm/build/ 开启]");
        }
        // 7. 支付平台
        if (message.contains("pay_")) {
            log.error("[支付模块 zrk-module-pay - 表结构未导入][参考 https://cloud.zrkcoder.cn/pay/build/ 开启]");
            return CommonResult.error(NOT_IMPLEMENTED.getCode(),
                    "[支付模块 zrk-module-pay - 表结构未导入][参考 https://cloud.zrkcoder.cn/pay/build/ 开启]");
        }
        // 8. AI 大模型
        if (message.contains("ai_")) {
            log.error("[AI 大模型 zrk-module-ai - 表结构未导入][参考 https://cloud.zrkcoder.cn/ai/build/ 开启]");
            return CommonResult.error(NOT_IMPLEMENTED.getCode(),
                    "[AI 大模型 zrk-module-ai - 表结构未导入][参考 https://cloud.zrkcoder.cn/ai/build/ 开启]");
        }
        // 9. IoT 物联网
        if (message.contains("iot_")) {
            log.error("[IoT 物联网 zrk-module-iot - 表结构未导入][参考 https://doc.zrkcoder.cn/iot/build/ 开启]");
            return CommonResult.error(NOT_IMPLEMENTED.getCode(),
                    "[IoT 物联网 zrk-module-iot - 表结构未导入][参考 https://doc.zrkcoder.cn/iot/build/ 开启]");
        }
        return null;
    }
