package cn.zrkcoder.cloud.framework.mybatis.config;
...
public class IdTypeEnvironmentPostProcessor implements EnvironmentPostProcessor {
    ...
    /**
     * 根据数据库类型设置 Quartz 任务存储的驱动类。
     * 如果环境中已存在驱动类配置，则直接返回；否则根据数据库类型动态设置对应的驱动类。
     *
     * @param environment 配置环境对象，用于读取和设置系统属性
     * @param dbType      数据库类型，决定使用哪种驱动类
     */
