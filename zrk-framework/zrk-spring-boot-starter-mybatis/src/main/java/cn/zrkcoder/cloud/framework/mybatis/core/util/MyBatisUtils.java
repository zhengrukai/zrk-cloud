package cn.zrkcoder.cloud.framework.mybatis.core.util;
...
import cn.zrkcoder.cloud.framework.common.pojo.SortingField;
import cn.zrkcoder.cloud.framework.mybatis.core.enums.DbTypeEnum;
...
public class MyBatisUtils {
    ...
    public static <T> Page<T> buildPage(PageParam pageParam, Collection<SortingField> sortingFields) {
        ...
        // 禁用 MyBatis-Plus 在执行分页查询时对 COUNT SQL 的 JOIN 优化。
        // 在某些复杂业务场景下，这种自动优化可能导致 COUNT 结果不准确（比如 JOIN 后数据膨胀或去重问题）。
        ...
        if (wrapper instanceof QueryWrapper<T>) {
            ...
            for (SortingField sortingField : sortingFields) {
                ...
                orderBy.append(StrUtil.toUnderlineCase(sortingField.getField()))
                        .append(" ")
                        .append(SortingField.ORDER_ASC.equals(sortingField.getOrder()) ? StrUtil.toUpperCase(SortingField.ORDER_ASC) : StrUtil.toUpperCase(SortingField.ORDER_DESC));
            }
