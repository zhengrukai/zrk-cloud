...
import cn.zrkcoder.cloud.module.infra.enums.codegen.CodegenColumnHtmlTypeEnum;
import cn.zrkcoder.cloud.module.infra.enums.codegen.CodegenColumnListConditionEnum;
...
public class CodegenBuilder {
    ...
    private void initTableDefault(CodegenTableDO table) {
        ...
        // 第一步，第一个 _ 前缀的后面，作为 business 名字; 第二步，可能存在多个 _ 的情况，转换成驼峰; 第三步，businessName 必须小写；
    ...
    private void processColumnExample(CodegenColumnDO column) {
        ...
        if (StrUtil.endWithIgnoreCase(column.getJavaField(), "name")) {
            column.setExample(randomEle(new String[]{"张三", "李四", "王五", "赵六", "zrk"}));
            return;
        }
        // status
        ...
        if (StrUtil.endWithIgnoreCase(column.getColumnName(), "url")) {
            column.setExample("https://github.com/zhengrukai/zrk-cloud");
            return;
        }
        // reason
