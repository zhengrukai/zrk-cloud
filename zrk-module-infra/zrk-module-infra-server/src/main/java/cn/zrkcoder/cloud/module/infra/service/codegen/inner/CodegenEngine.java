package cn.zrkcoder.cloud.module.infra.service.codegen.inner;
...
import cn.zrkcoder.cloud.framework.common.util.date.DateUtils;
import cn.zrkcoder.cloud.framework.common.util.date.LocalDateTimeUtils;
import cn.zrkcoder.cloud.framework.common.util.object.BeanUtils;
import cn.zrkcoder.cloud.framework.common.util.object.ObjectUtils;
import cn.zrkcoder.cloud.framework.common.util.string.StrUtils;
...
import cn.zrkcoder.cloud.module.infra.enums.codegen.CodegenSceneEnum;
import cn.zrkcoder.cloud.module.infra.enums.codegen.CodegenTemplateTypeEnum;
import cn.zrkcoder.cloud.module.infra.enums.codegen.CodegenVOTypeEnum;
import cn.zrkcoder.cloud.module.infra.framework.codegen.config.CodegenProperties;
...
public class CodegenEngine {
    ...
    /**
     * 是否为 zrk-cloud 项目，用于解决 Boot 和 Cloud 的 api 模块兼容性问题
     * true  - 需要有 zrk-module-xxx-api 模块
    ...
    public CodegenEngine() {
        ...
        this.cloudEnable = ClassUtils.isPresent("cn.zrkcoder.cloud.module.infra.framework.rpc.config.RpcConfiguration",
    ...
    private Map<String, String> getTemplates(Integer frontType) {
        ...
        if (Boolean.FALSE.equals(cloudEnable)) {
            SERVER_TEMPLATES.forEach((templatePath, filePath) -> {
                filePath = StrUtil.replace(filePath, "/zrk-module-${table.moduleName}-api", "");
                filePath = StrUtil.replace(filePath, "/zrk-module-${table.moduleName}-server", "");
    ...
    private static String javaModuleFilePath(String path, String module, String src) {
        return "zrk-module-${table.moduleName}/" + // 顶级模块
                "zrk-module-${table.moduleName}-" + module + "/" + // 子模块
                "src/" + src + "/java/${basePackage}/module/${table.moduleName}/" + path + ".java";
    }

    private static String mapperXmlFilePath() {
        return "zrk-module-${table.moduleName}/" + // 顶级模块
                "zrk-module-${table.moduleName}-server/" + // 子模块
    ...
    private static String vueFilePath(String path) {
        return "zrk-ui-${sceneEnum.basePackage}-vue2/" + // 顶级目录
                "src/" + path;
    }
    ...
    private static String vue3FilePath(String path) {
        return "zrk-ui-${sceneEnum.basePackage}-vue3/" + // 顶级目录
                "src/" + path;
    }
    ...
    private static String vue3UniappFilePath(String path) {
        return "zrk-ui-${sceneEnum.basePackage}-uniapp/" + // 顶级目录
                "src/" + path;
    }

    private static String vue3VbenFilePath(String path) {
        return "zrk-ui-${sceneEnum.basePackage}-vben/" + // 顶级目录
                "src/" + path;
    }
