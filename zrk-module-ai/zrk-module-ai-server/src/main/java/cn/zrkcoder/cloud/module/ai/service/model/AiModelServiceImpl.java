package cn.zrkcoder.cloud.module.ai.service.model;
...
import cn.zrkcoder.cloud.module.ai.controller.admin.model.vo.model.AiModelPageReqVO;
import cn.zrkcoder.cloud.module.ai.controller.admin.model.vo.model.AiModelSaveReqVO;
import cn.zrkcoder.cloud.module.ai.dal.dataobject.model.AiApiKeyDO;
import cn.zrkcoder.cloud.module.ai.dal.dataobject.model.AiModelDO;
import cn.zrkcoder.cloud.module.ai.dal.mysql.model.AiModelMapper;
...
//         return modelFactory.getOrCreateVectorStore(MilvusVectorStore.class, embeddingModel, metadataFields);
    ...
    public void getLLmProvider4Tinyflow(Tinyflow tinyflow, Long modelId) {
        ...
        AiPlatformEnum platform = AiPlatformEnum.validatePlatform(apiKey.getPlatform());
        switch (platform) {
            // TODO 考虑到未来不需要使用agents-flex 现在仅测试通义千问
            // TODO 【重要】是不是可以实现一个 SpringAiLlm，这样的话，内部全部用它就好了。只实现 chat 部分；这样，就把 flex 作为一个 agent 框架，内部调用，还是 spring ai 相关的。成本可能低一点？！
            case TONG_YI:
